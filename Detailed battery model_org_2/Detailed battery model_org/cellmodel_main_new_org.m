...
C2_p        = Ce0*ones(1,N_p)/Ce_sc;
C2_m        = Ce0*ones(1,N_m)/Ce_sc;
C2_n        = Ce0*ones(1,N_n)/Ce_sc;

Jptot       = I_app/(as_p*S_p*L_p);
Jntot       = -I_app/(as_n*S_n*L_n);

C3_p        = C1_p*Csp_sc/Csep_sc -(Jptot*Rs_p/(5*F*Ds_p*Csep_sc));
C3_n        = C1_n*Csn_sc/Csen_sc -(Jntot*Rs_n/(5*F*Ds_n*Csen_sc));
...
y           = [C1_p(end,:), C1_n(end,:), C2_p(end,:),C2_m(end,:),C2_n(end,:),P1p0, P1n0, P20,C3_p(end,:),C3_n(end,:), Jpi_init, Jni_init];
...
for ndx = 1:N_cycles
    ...
    y0_all          = [y(end,:), y(end,end-N_n+1:end)];
    
    options         = odeset('Mass',@massmatrix_gen_DAE,'MaxStep',100,'RelTol',1e-2,'Events',@breakode_battery);
    [t,y]           = ode23t(@battery_pde_model_DAE,[0 t_max/t_sc],y0_all,options,Index_chdisch, I_app,[], R_sei, R_plating, Cs_max_p,Cs_max_n,epse_m);
...
%% CC charging with Li plating (Index_chdisch = 4)
    ...
    if t(end)*t_sc < t_cc_char
        Index_chdisch   = 4;
        y0_all          = [y(end,:), y(end,end-N_n+1:end)];
        
        options         = odeset('Mass',@massmatrix_gen_DAE,'MaxStep',100,'RelTol',1e-2,'Events',@breakode_battery);
        [t,y]           = ode23t(@battery_pde_model_DAE,[0 (t_cc_char - t(end)*t_sc)/t_sc],y0_all,options,Index_chdisch, I_app,[], R_sei, R_plating, Cs_max_p,Cs_max_n,epse_m);
        time            = [time; time(end) + (t*t_sc)];
        ...
        Jni_des         = y(:,6*N_p+2*N_m+5*N_n+1:6*N_p+2*N_m+6*N_n)*abs(I_app/S_n);
        Jni_sei         = y(:,6*N_p+2*N_m+6*N_n+1:6*N_p+2*N_m+7*N_n)*abs(I_app/S_n);
        Jni_sei_plating = y(:,6*N_p+2*N_m+7*N_n+1:6*N_p+2*N_m+8*N_n)*abs(I_app/S_n);
    ...
    options = odeset('Mass',@massmatrix_gen_DAE,'RelTol',1e-2,'MaxStep',100,'Events',@breakode_battery);
    [t,y]   = ode23t(@battery_pde_model_DAE,[0 0.25*t_max/t_sc],y0_all,options,Index_chdisch,[],Vcell(end),R_sei, R_plating, Cs_max_p,Cs_max_n,epse_m);
    ...
    Jni_des   = y(:,6*N_p+2*N_m+5*N_n+1:6*N_p+2*N_m+6*N_n).*abs(repmat(I_app,1,N_n)/S_n);
    Jni_sei   = y(:,6*N_p+2*N_m+6*N_n+1:6*N_p+2*N_m+7*N_n).*abs(repmat(I_app,1,N_n)/S_n);
    ...
    options = odeset('Mass',@massmatrix_gen_DAE,'RelTol',1e-2,'MaxStep',100,'Events',@breakode_battery);
    [t,y]   = ode15s(@battery_pde_model_DAE,[0 t_max/t_sc],y0_all,options,Index_chdisch,I_app,[],R_sei, R_plating, Cs_max_p,Cs_max_n,epse_m);
...
time = time(2:end);
toc
% clearvars -except N_cycles time Vcell Current C_loss_sei_all del_sei_all R_sei_all Li_den_matrix R_plating_all C_loss_plating_all epse_m_all
...
% ylabel('Cell voltage (V)')
% 
% % save(['Results_',N_cycles,'cycles_with_SEI_plating'])
