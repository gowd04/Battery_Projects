function M  = massmatrix_gen_DAE(~,~,Index_chdisch,~,~,~,~,~,~,~,~,~, ~)
    ...
    if Index_chdisch == 1
        M = diag(ones(7*(N_p+N_n) + 3*N_m ,1));
    else
        M = diag(ones(7*(N_p+N_n) + 3*N_m +N_n,1));
    end
    ...
    % Time derivative of temperature at boundaties is zero
    M(5*N_p + 2*N_m + 5*N_n+1) = 0;
    M(6*N_p+3*N_m+6*N_n) = 0;
    % Current density equations    
    
    if Index_chdisch == 1
        for jdx = 6*N_p + 3*N_m + 6*N_n+1:7*N_p + 3*N_m + 7*N_n
            M(jdx,jdx) = 0;
        end
    elseif Index_chdisch == 4
        for jdx = 6*N_p + 3*N_m + 6*N_n+1:7*N_p + 3*N_m + 9*N_n
            M(jdx,jdx) = 0;
        end    
    else
        for jdx = 6*N_p + 3*N_m + 6*N_n+1:7*N_p + 3*N_m + 8*N_n
            M(jdx,jdx) = 0;
        end        
    end
  
    if Index_chdisch == 3
        M(7*N_p + 3*N_m + 8*N_n+1,7*N_p + 3*N_m + 8*N_n+1) = 0;
    end
end
