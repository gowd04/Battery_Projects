...
for i = 1:length(voltages)
    ...
    expt = expt + 1 ;
    disp(['experiment No: ', num2str(expt)])
    tic
    % cd E:\Research\Codes\ElectroThermal battery model
    parameters2_NMC
    initial_condns
...
%     EOCV     = 4.2 + ((4.4 - 4.2)*rand(1,1));
%     T_init   = 288.15 + ((318.15 - 288.15)*rand(1,1));
%     C_rate   = 0.5 + ((1.5 - 0.5)*rand(1,1));

    EOCV     = voltages(i);
    T_init   = Temperatures(i);
    C_rate   = C_rates(i);
    
    EOCV_store          = [EOCV_store; EOCV];
    T_init_store        = [T_init_store; T_init];
    C_rate_store        = [C_rate_store; C_rate];

    T0       = T_init;        % initial condition for temperature(K)
    phie_sc  = R*T_init/F;    % Scale for electrolyte potential (V)

    I_app       = C_rate*rated_curr;
...
% Equations obtained from 'Modeling of lithium plating and lithium stripping in lithium-ion batteries'

    Uref0_p = 6.0826 - 6.9922*theta_p0 + 7.1062*(theta_p0.^2) - 0.54549e-4*exp((124.23 * theta_p0) - 114.2593) - (2.5947 * theta_p0.^3);

    Uref0_n = 0.6379 + 0.5416*exp(-305.5309*theta_n0) + 0.044*tanh(-(theta_n0 - 0.1958)/0.1088) - 0.1978*tanh((theta_n0 - 1.0571)/0.0854) - 0.6875*tanh((theta_n0 + 0.0117)/0.0529)...
    - 0.0175*tanh((theta_n0 - 0.5692)/0.0875);
...
%         disp(['Cycle No. = ',num2str(ndx)])
    ...
    %% CC charging with Li plating (Index_chdisch = 4)
        ...
        [t,y]           = ode23t(@battery_pde_model_DAE_new2,[0 t_max/t_sc], y0_all,options,Index_chdisch, I_app,[], R_sei, R_plating, Cs_max_p,Cs_max_n,epse_m, Ta, EOCV, T_init);
...
%     end
        ...
        [t,y]   = ode23t(@battery_pde_model_DAE_new2,[0 0.25*t_max/t_sc],y0_all,options,Index_chdisch,[],Vcell(end),R_sei, R_plating, Cs_max_p,Cs_max_n,epse_m, Ta, EOCV, T_init);
        ...
        Temp_avg    = mean(Temp_cell');
        Temp        = Temp_avg';
        charging_time_dur_cycle = time(end);
        max_temp_dur_cycle      = max(Temp_avg);
        Temp_rise               = max_temp_dur_cycle - T_init;
        dendrite_shape
    %     Ta          = Temp_avg(end);
        C_loss_sei_store    = [C_loss_sei_store; C_loss_sei];
        C_loss_pt_store     = [C_loss_pt_store; C_loss_plating];
        dendrite_len_store  = [dendrite_len_store; max_len];
        temp_rise_store     = [temp_rise_store; Temp_rise];
        charging_time_store = [charging_time_store;charging_time_dur_cycle];
...
%         
%         options = odeset('Mass',@massmatrix_gen_DAE,'RelTol',1e-2,'MaxStep',100,'Events',@breakode_battery_new);
%         [t,y]   = ode15s(@battery_pde_model_DAE_new2,[0 t_max/t_sc],y0_all,options,Index_chdisch,I_app,[],R_sei, R_plating, Cs_max_p,Cs_max_n,epse_m, Ta, EOCV, T_init);
%         
%         time = [time; time(end) + (t*t_sc)];
%         
%         C1_p = y(:,1:N_p);
...
%         Temp        = Temp_avg';
%     %     Ta          = Temp_avg(end);
%         temp_eoc       = [temp_eoc;Temp_avg(end)];
    
    end
%     clearvars -except charging_time_store C_loss_pt_store C_loss_sei_store dendrite_len_store temp_rise_store voltages Temperatures C_rates i
end
time = time(2:end);
Simulation_time = toc;
% dendrite_shape
...
% save(['Results_',num2str(N_cycles),'cycles.mat'])
Simulation_time
%%
% figure
% plot(Psp(:,end))
