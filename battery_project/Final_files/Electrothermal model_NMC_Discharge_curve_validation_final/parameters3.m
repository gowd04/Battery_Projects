%% Paper: Modeling of lithium plating and lithium stripping in lithium-ion batteries %%
%% Geometric parameters

L_p     = 67e-6;   % Length of positive electrode (m)
L_m     = 25e-6;   % Length of membrane separator (m)
L_n     = 79e-6;   % Length of negative electrode (m)
L       = L_p + L_m + L_n;  % Total length (m)

S_p     = 0.06;    % Geometric surface area of positive electrode (m^2)
S_n     = 0.06;    % Geometric surface area of negative electrode (m^2)

Rs_p    = 4.6e-6;     % Radius of particle in positive electrode (m)
Rs_n    = 10.5e-6;     % Radius of particle in negative electrode (m)

epse_p  = 0.3;    % Volume fraction of electrolyte in positive electrode (-)
epse_m0 = 0.45;     % Volume fraction of electrolyte in separator (-)
epse_n  = 0.3;    % Volume fraction of electrolyte in negative electrode (-)
...
epss_p  = 0.56;   % Volume fraction of solid active material in positive electrode (-)
epss_n  = 0.56;   % Volume fraction of solid active material in negative electrode (-)
...
Cs_max_p0   = 51385;    % Maximum concentration of solid phase lithium in positive electrode (mol/m^3)
Cs_max_n0   = 31370;    % Maximum concentration of solid phase lithium in negative electrode (mol/m^3)
...
% Ea_dp       = 15e3;      %activation energy for particle diffusion for positive electrode (J/mol);
% Ea_dn       = 15e3;      %activation energy for particle diffusion for negative electrode (J/mol);
% Ea_rp       = 3.5e4;     %activation energy for reaction for positive electrode (J/mol);
% Ea_rn       = 3.5e4;     %activation energy for reaction for negative electrode (J/mol);
% Ea_plating  = 3.53e4;
...
Ds_p_ref    = 1e-14;    % Diffusion coefficient of solid phase in positive electrode (m^2/s)
Ds_n_ref    = 1e-14;  % Diffusion coefficient of solid phase in negative electrode (m^2/s)
...
sigmas_p    = 3.8;       % Intrinsic conductivity of active materials in solid phase of positive electrode (1/(Ohm m))
...
transf_p      = 0.38*ones(1,42);      % Transference number (-)
transf_m      = 0.38*ones(1,14);
transf_n      = 0.38*ones(1,42);
...
kp_ref          = 2.5e-10;         % Rate constant for positive electrode (m/s)
kn1_ref         = 2.2e-6;        % Rate constant for negative electrode (m/s)
kn2_ref         = 2.5e-12;        % Rate constant for negative electrode (m/s), shif
% % kn_plating_ref  = 0;                % Rate constant for Li plating in negative electrode (m/s)
% % kn_sei_ref      = 0; 
% kn_sei_ref      = 30*1.1e-20;    % Rate constant for SEI layer formation in negative electrode (m/s)
% kn_plating_ref  = 20*1.1e-20;     % Rate constant for Li plating in negative electrode (m/s)
...
kn_sei_ref      = 1.1e-17;    % Rate constant for SEI layer formation in negative electrode (m/s)
kn_plating_ref  = 1.1e-18;    % Rate constant for Li plating in negative electrode (m/s)
...
EOCC        = 0.05;       % End of charging current (A)
...
rho_p       = 2328.5; %kg/m^3
% rho_p       = 2000; %kg/m^3 (Tap density obtained from paper:  Dennis W. Dees et al 2015 J. Electrochem. Soc. 162 A559)
rho_n       = 1347.3; %kg/m^3
rho_m       = 1123.0; %kg/m^3
% rho_p       = 2292; %kg/m^3
