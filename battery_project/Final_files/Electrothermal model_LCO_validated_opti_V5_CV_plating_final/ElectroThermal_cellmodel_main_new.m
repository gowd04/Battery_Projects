...
%     C_rate   = 0.5 + ((1.5 - 0.5)*rand(1,1));
    ...
    for ndx = 1:N_cycles
%         disp(['Cycle No. = ',num2str(ndx), '/' ,num2str(N_cycles)])
        charging_time = 0;
...
%     end
        ...
        time = [time; time(end) + (t*t_sc)];
        charging_time = charging_time + t(end)*t_sc;
        ...
        Vcell       = [Vcell; P1_p(:,1)*phisp_sc - P1_n(:,end)*phisn_sc];
        Current     = [Current; y(:,7*N_p+3*N_m+8*N_n+1)];
        Ce          = [Ce; [C2_p, C2_m,C2_n]*Ce_sc];
        Pe          = [Pe; [P2_p, P2_m,P2_n]*phie_sc];
        Csp         = [Csp; C1_p*Csp_sc];
        Csn         = [Csn; C1_n*Csn_sc];
    ...
    %     Ta          = Temp_avg(end);
        ...
        charging_time_store = [charging_time_store;charging_time];
...
%         discharge_time_store = [discharge_time_store;discharge_time];
