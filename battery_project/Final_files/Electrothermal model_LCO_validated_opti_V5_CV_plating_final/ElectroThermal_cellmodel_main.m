...
C_loss_side_all     = [0 C_loss_side];      % C_loss_side represents loss of lithium due to SEI as well as plating
...
Cs_max_p    = Cs_max_p0 - C_loss_side;
Cs_max_n    = Cs_max_n0 - C_loss_side;
...
%     
%     delta_sei       = sum(abs(Jni_side-Jni_des)*M_sei.*repmat([0;diff(t*t_sc)],1,N_n)/(rho_sei*F)) + delta_sei;
%     R_sei           = delta_sei/k_cond_sei;
%     
%     C_loss_side      = sum(sum(abs(Jni_side-Jni_des).*repmat([0;diff(t*t_sc)],1,N_n)*as_n/(F))) + C_loss_side;
%     
%     Cs_max_p        = Cs_max_p0 - C_loss_side;
%     Cs_max_n        = Cs_max_n0 - C_loss_side;
%     
...
%     C_loss_side_all  = [C_loss_side_all; time(end) C_loss_side];
...
%% CC charging with Li plating (Index_chdisch = 4)
        ...
        del_sei_all     = [del_sei_all; time(end) delta_sei];
        R_sei_all       = [R_sei_all; time(end) R_sei];
        Temp_cell       = [Temp_cell;[T_p T_s T_n]*Ta];
...
%     end
    ...
    Jni_side   = y(:,7*N_p+3*N_m+7*N_n+1:7*N_p+3*N_m+8*N_n).*abs(repmat(I_app,1,N_n)/S_n);
