function [value,isterminal,direction] = breakode_battery(t,y,Index_chdisch,~,~,~,~,Cs_max_p,Cs_max_n,~)
    ...
    elseif Index_chdisch == 2 %% CC charging
        
        if (sign(P1_p(1) - P1_n(end) - EOCV) - 1) == 0 || ~isempty(find(C1_n_act >= 0.83, 1)) || ~isempty(find(C1_p_act <= 0.4701, 1))
            value = 0;
        else
            value = 1;
        end
    ...
    elseif Index_chdisch == 3 %% CV charging
        ...
        if (sign(I_app - EOCC) + 1) == 0 || ~isempty(find(C1_n_act >= 0.83, 1)) || ~isempty(find(C1_p_act <= 0.4701, 1))
            value = 0;
        else
            value = 1;
        end
