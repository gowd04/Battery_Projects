function dydt = battery_pde_model_DAE(~,y,Index_chdisch,I_app,Vcell_cc,R_sei, R_plating,Cs_max_p,Cs_max_n,epse_m, Ta, EOCV, T_init)
...
% Ds_p = Ds_p_ref.*(ones(1,N_p)');
De_p = De_p_ref.*(ones(1,N_p)');
% kappae_p = kappae_p_ref.*(ones(1,N_p)');
...
De_m = De_m_ref.*(ones(1,N_m)');
% kappae_m = kappae_m_ref.*(ones(1,N_m)');
...
% Ds_n = Ds_n_ref.*(ones(1,N_n)');
De_n = De_n_ref.*(ones(1,N_n)');
% kappae_n = kappae_n_ref.*(ones(1,N_n)');
...
% equations obtained from paper "Single-Particle Model for a Lithium-Ion Cell: Thermal Behavior"
